...
xkb = U.lazyload(function()
	...
	assert(table ~= nil, 'could not create XKB compose table for locale: ' .. locale)
...
---@return 'text'|'waiting'|'command' kind
---@return nil|string text UTF-8 character/string when kind is `text`
function M.process_next_input(keysym)
	...
	-- 0 is XKB_COMPOSE_FEED_IGNORED, 1 is ACCEPTED: only 0 returns early here
...
-- printable chars stay lazy: each first use resolves through key_map
-- __index below, so requiring this module never loads xkbcommon by itself
...
local mods = { Ctrl = 'C', Alt = 'A', Shift = 'S' }

---@param bind string
---@return string
function M.short_key_name(bind)
	bind = bind:gsub('([^+-]+)[+-]', function(x) return (mods[x] or x) .. '-' end):gsub('ISO_Left_Tab$', 'Tab')

	local key = bind:match '[^+-]*.$'
	if #bind == 1 and key:match '[%d%l]' then return key end
	-- keep the readable xkb name for anything below a printable char (space):
	-- non-printable keysyms (Home, F1, Tab) map via xkb_to_utf8 to a control
	-- char or '\0', which would truncate or mangle the rendered string
	...
	-- anything else (a single char, an abbreviation, a function key) is bracketed,
	-- so the shortened form cannot be mistaken for literal text
