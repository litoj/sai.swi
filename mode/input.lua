...
function M:get_current_line_info()
	local i = 1
	for l in self._text:gmatch '([^\n]*)\n?' do
		if self._col <= i + #l then return { line = l, from = i, to = i + #l } end
		i = i + #l + 1
	end
	return { line = '', from = #self._text + 1, to = #self._text + 1 }
end
...
function M:set_text(val)
	...
	for i = self._col - 1, 1, -1 do -- check up to the cursor position
...
function M:set_line(val)
	...
	self.col = lines[math.min(#lines - 1, math.max(1, val) - 1)] + col + 1
	return false
end
